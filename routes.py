...
def verify_recaptcha(token):
    try:
        ...
        logger.debug(f"Verifying reCAPTCHA token: {token[:10]}..."
        ...
        logger.debug(
            f"Request payload: secret=<redacted>, response length={len(token)}, remoteip={request.remote_addr}"
        )
        ...
        if not result.get('success'):
            ...
            logger.warning(
                f"reCAPTCHA verification failed with error codes: {error_codes}"
            )
            return False

        # Log the full verification details
        logger.info(
            f"reCAPTCHA verification details: score={result.get('score')}, action={result.get('action')}, timestamp={result.get('challenge_ts')}"
        )

        if result.get('action') != 'register':
            logger.warning(
                f"reCAPTCHA action mismatch. Expected 'register', got: {result.get('action')}"
            )
            return False
    ...
    if request.method == 'POST':
        try:
            ...
            if not recaptcha_token:
                flash(
                    'Please ensure JavaScript is enabled for reCAPTCHA validation'
                )
            ...
            logger.info(
                f"Generating verification token for user {user.username}")
            token = user.generate_verification_token()

            db.session.add(user)
            db.session.commit()
            logger.info(
                f"User {user.username} added to database with token: {token[:10]}..."
            )
...
def dashboard():
    ...
    recent_articles = Article.query.join(Feed).filter(
        Feed.user_id == current_user.id).order_by(
            nullslast(desc(Article.published_date))).limit(10).all()
...
def manage_feeds():
    if request.method == 'POST':
        ...
        new_feed = Feed(
            ...
            urlparse(feed_url).netloc[:200],  # Truncate to 200 chars
...
def feed_health_dashboard():
    ...
    total_articles = sum(
        getattr(feed, 'total_articles_processed', 0) or 0 for feed in feeds)
...
def import_opml():
    ...
    try:
            ...
            for entry in outline:
                ...
                if hasattr(entry, 'xmlUrl'):
                    ...
                    existing_feed = Feed.query.filter_by(
                        url=entry.xmlUrl, user_id=current_user.id).first()

                    if not existing_feed:
                        new_feed = Feed(
                            url=entry.xmlUrl,
                            title=(getattr(entry, 'text', '') or getattr(
                                entry, 'title',
                ...
                if len(
                        entry
                ) > 0:  # Using Python sequence operator to check for nested elements
                    process_outline(
                        entry)  # Recursively process nested outlines
        ...
        flash(
            f'Successfully imported {imported_count} feeds ({skipped_count} skipped as duplicates)'
        )
...
def webhook_feed_updated():
    ...
    if request.method == 'GET':
        """The WebHook will need to be verified."""
        mode = request.args.get('mode')
        token = request.args.get('token') or ""

        logger.info(f"Webhook verification called {mode} {token}")

        if mode == 'verify':
            return make_response(token, 200)

        return make_response("Invalid mode", 500)
    ...
    if request.method == 'POST':
        try:
            ...
            response = jsonify({
                'status': 'success',
                'message': 'Feed update queued for processing'
            })
